...
export const EntityVerification: React.FC<EntityVerificationProps> = ({
  ...
  const handleVerify = async () => {
    ...
    try {
      const response = await api.get<RiskDetails>(
        `/v1/risk/verify?query=${encodeURIComponent(query)}`,
      );
      ...
      setError(
        err.response?.data?.detail ||
          "Verification failed. Service may be unavailable.",
      );
    } finally {
      setVerifying(false);
    }
    ...
    <div className="space-y-4 mb-6 border border-slate-200 rounded-xl p-5 bg-slate-50/50">
      <div className="flex flex-col space-y-1.5 mb-2">
        ...
        <p className="text-xs text-slate-500">
          Enter an Israeli Company ID or Entity Name. We'll automatically verify
          against local registries and global AML/KYC sanctions lists.
        </p>
      </div>
            ...
            <div className="bg-red-50 border border-red-200 rounded-lg p-3 flex gap-3 text-red-900 text-sm">
              ...
              <div className="space-y-2">
                <p className="font-bold text-red-700">
                  ⚠️ High Risk Entity Detected
                </p>

                {result.local_risk?.is_high_risk && (
                  <div className="text-xs bg-red-100/50 p-2 rounded">
                    <span className="font-semibold block mb-0.5">
                      Local Registry Alert:
                    </span>
                    Status is <strong>{result.local_risk.status}</strong>.{" "}
                    {result.local_risk.risk_reason}
                  </div>
                )}
                ...
                {result.global_risk?.is_high_risk && (
                  <div className="text-xs bg-red-100/50 p-2 rounded">
                    <span className="font-semibold block mb-0.5">
                      Global AML/KYC Sanctions Alert:
                    </span>
                ...
                {!result.local_risk?.is_high_risk &&
                  !result.global_risk?.is_high_risk && (
                    <div className="text-xs">
                      This entity was flagged for manual review reasons.{" "}
