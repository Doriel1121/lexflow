          ...
          <tbody className="divide-y divide-slate-50">
            {sorted.map((emp) => (
              ...
              >
                ...
                <td className="px-4 py-3">
                  {emp.deadline_compliance_rate === null ||
                  emp.total_deadlines === 0 ? (
                    <span className="text-xs text-slate-400">N/A</span>
                  ) : (
                    <span
                      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold ${complianceColor(emp.deadline_compliance_rate)}`}
                    >
