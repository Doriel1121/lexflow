...
export default function AdminDashboard() {
  ...
  const roleData = Object.entries(user_stats.users_by_role).map(
    ([role, count]) => ({
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard
          label={t("adminDashboard.totalTenants")}
        ...
        <KPICard
          ...
          iconBg="bg-blue-100"
          trend={{
            value: user_stats.new_users_today,
            label: t("adminDashboard.today"),
          }}
        />
        <KPICard
          label={t("adminDashboard.documentsProcessed")}
        ...
        />
        <KPICard
          label={t("adminDashboard.activeCases")}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard
          label={t("adminDashboard.aiCallsToday")}
        ...
        <KPICard
          ...
          subtext={`${activity_stats.api_errors_today} ${t("adminDashboard.errors").replace("{{count}}", "")}`}
          icon={Activity}
          iconColor="text-sky-600"
          iconBg="bg-sky-100"
        />
        <KPICard
          label={t("adminDashboard.errorRate")}
          value={`${activity_stats.error_rate_pct.toFixed(1)}%`}
          subtext={
            activity_stats.error_rate_pct > 3
              ? t("adminDashboard.aboveThreshold")
              : t("adminDashboard.withinNormal")
          }
          icon={AlertTriangle}
          iconColor={
            activity_stats.error_rate_pct > 3
          ...
          iconBg={
            activity_stats.error_rate_pct > 3 ? "bg-red-100" : "bg-slate-100"
          }
        />
        <KPICard
          label={t("adminDashboard.avgLatency")}
          value={`${activity_stats.avg_latency_ms.toFixed(0)} ms`}
          subtext={`P95: ${activity_stats.p95_latency_ms.toFixed(0)} ms`}
          icon={Clock}
          iconColor={
            activity_stats.avg_latency_ms > 1000
              ? "text-amber-600"
              : "text-teal-600"
          }
          iconBg={
