...
export function CollectionView() {
  ...
  const getCategoryIcon = (category?: string) => {
    switch (category) {
      ...
      case "project":
        return <FolderGit2 className="h-4 w-4 mr-2" />;
      case "organization":
      ...
      default:
        return <TagIcon className="h-4 w-4 mr-2" />;
    }
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-500 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex items-center justify-between mt-2">
          <div className="flex items-center gap-3">
            <div
              className={`p-2.5 rounded-lg border ${getCategoryTheme(tag.category)}`}
            >
          ...
          <span
            className={`text-xs font-semibold px-3 py-1 rounded-full border flex items-center ${getCategoryTheme(tag.category)}`}
          >
            {getCategoryIcon(tag.category)}
            {tag.category
              ? tag.category.charAt(0).toUpperCase() +
                tag.category.slice(1).replace("_", " ")
              : "General"}
          </span>
        </div>
        <p className="text-muted-foreground">
          Showing all documents the AI automatically categorized under this
          collection.
        </p>
      </div>
      ...
      {documents.length === 0 ? (
        <div className="text-center py-16 bg-card rounded-xl border border-dashed text-muted-foreground">
          ...
          {t("collections.noDocumentsInCollection")}
        </div>
      ) : (
