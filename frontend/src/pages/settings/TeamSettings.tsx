...
export default function TeamSettings() {
  ...
  const fetchTeamMembers = async () => {
    try {
      setLoading(true);
      // First get current user to retrieve organization ID
      const meRes = await api.get("/v1/users/me");
      const orgId = meRes.data.organization_id;

      if (!orgId) {
        console.error("Current user has no organization");
        setMembers([]);
        return;
      }

      // Fetch members of the current organization only
      const res = await api.get(`/v1/organizations/${orgId}/members`);
      setMembers(res.data);
    } catch (err) {
        ...
        <div className="md:col-span-1 space-y-6">
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
            ...
            <form onSubmit={handleInvite} className="space-y-4">
              ...
              {inviteMessage && (
                <div
                  className={`p-3 rounded-lg text-sm ${inviteMessage.type === "success" ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-red-50 text-red-700 border border-red-200"}`}
                >
                  {inviteMessage.text}
                </div>
              )}
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-semibold text-slate-800">
                          {member.full_name || t("team.pendingUser")}
                        </p>
