...
const SettingsPage: React.FC = () => {
  ...
  const fetchOrgSettings = async () => {
    try {
      const response = await api.get(
        `/v1/organizations/${user?.organization?.id}`,
      );
  ...
  const toggleBatterySave = async () => {
    ...
    try {
      ...
      showSnackbar(
        `Failed to update AI settings: ${error.response?.data?.detail || error.message}`,
        { type: "error" },
      );
    } finally {
  ...
  const handleAddEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showSnackbar(
        `Failed to add email: ${error.response?.data?.detail || error.message}`,
        { type: "error" },
      );
    } finally {
      setAdding(false);
  ...
  const handleDelete = async (id: number) => {
    const ok = await confirm(
      t("settingsPage.deleteEmailConfirm", {
        defaultValue: "Remove this email intake address?",
      }),
      {
        variant: "danger",
  ...
  const handleToggle = async (id: number) => {
    try {
      ...
      setEmailConfigs((c) =>
        c.map((cfg) =>
          cfg.id === id
            ? { ...cfg, ingestion_enabled: res.data.ingestion_enabled }
            : cfg,
        ),
      );
    } catch (error: any) {
      showSnackbar(
        `Toggle failed: ${error.response?.data?.detail || error.message}`,
        { type: "error" },
      );
    }
  };
      ...
      <div className="bg-white rounded-2xl p-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <span
              className={`inline-block mt-1 px-2 py-0.5 rounded-full text-xs font-semibold ${roleColor[user?.role ?? ""] ?? "bg-slate-100 text-slate-700"}`}
            >
              {roleLabel}
            </span>
          </div>
        ...
        <div className="bg-white rounded-2xl p-6 border-2 border-emerald-50">
          <div className="flex items-center justify-between">
            <div className="flex items-start gap-4">
              <div
                className={`p-3 rounded-xl ${batterySave ? "bg-emerald-100 text-emerald-600" : "bg-amber-100 text-amber-600"}`}
              >
                {batterySave ? (
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1 max-w-md">
                  When enabled, AI will only analyze a sample of document pages
                  to save API tokens and reduce costs. Best for large documents
                  and restricted quotas.
                </p>
              </div>
            </div>
            <button
      ...
      <div className="bg-white rounded-2xl p-6">
        ...
        {showAddForm && (
          <form
            ...
            className="mb-5 p-4 bg-slate-50 rounded-xl border border-slate-100"
          >
            ...
            <div className="flex gap-2">
              ...
              >
                {adding
                  ? t("settingsPage.addForm.adding")
                  : t("settingsPage.addForm.add")}
              </button>
              <button
                type="button"
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {emailConfigs.map((cfg) => {
              const inboundAddress = cfg.inbound_slug
                ? `${cfg.inbound_slug}@${INBOUND_DOMAIN}`
                : null;
              ...
              return (
                <div
                  key={cfg.id}
                  className={`rounded-xl border p-4 transition-all ${cfg.ingestion_enabled ? "border-slate-100 bg-slate-50/50" : "border-dashed border-slate-200 opacity-60"}`}
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-4 mt-2.5 text-xs text-slate-400">
                        <span className="flex items-center gap-1">
                          ...
                          {t("settingsPage.docsIngested", {
                            count: cfg.total_ingested ?? 0,
                          })}
                        </span>
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button
                        ...
                        title={
                          cfg.ingestion_enabled
                            ? t("settingsPage.pauseIngestion")
                            : t("settingsPage.enableIngestion")
                        }
                      ...
                      <button
                        ...
                        title={t("settingsPage.removeEmail")}
