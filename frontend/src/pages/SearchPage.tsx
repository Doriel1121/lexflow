...
import { 
  Search, Loader2, FileText, Tag, Link, Briefcase, 
  Users, ChevronRight, AlertCircle, FileSearch 
} from 'lucide-react';
...
const SearchPage: React.FC = () => {
  ...
  const performSearch = useCallback(async (searchTerm: string) => {
    ...
    setError(null);
    try {
      const response = await api.get<GlobalSearchResult>(`/v1/search?query=${encodeURIComponent(searchTerm)}`);
  ...
  const hasResults = results && (results.cases.length > 0 || results.documents.length > 0 || results.clients.length > 0);
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-500">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-slate-500 text-sm">Unified search across clients, cases, and documents.</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="flex gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search for anything (case name, client, document)..."
              value={query}
      ...
      {loading && !results && (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <p className="text-slate-500 font-medium">Scanning your legal database...</p>
        </div>
      )}
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="font-bold text-slate-800 group-hover:text-primary transition-colors">{client.name}</h3>
                          <p className="text-xs text-slate-500 truncate max-w-[150px]">{client.contact_email || 'No email'}</p>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <span className="text-xs text-slate-400">Created {new Date(caseItem.created_at).toLocaleDateString()}</span>
                        </div>
                      </div>
                    </div>
                    <ChevronRight className="h-4 w-4 text-slate-300 group-hover:text-primary transition-colors" />
                ...
                {results.documents.map(doc => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="font-bold text-slate-800 group-hover:text-primary transition-colors">{doc.filename}</h3>
                        <div className="flex items-center gap-3 mt-1">
                          ...
                          {doc.case_id && (
                            <span className="text-xs text-slate-400">Case #{doc.case_id}</span>
                          )}
                        </div>
                      </div>
                    </div>
          ...
          {!hasResults && !loading && (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-slate-500 text-sm max-w-xs mt-1">
                We couldn't find any clients, cases, or documents matching "{query}".
              </p>
            </div>
          )}
        </div>
