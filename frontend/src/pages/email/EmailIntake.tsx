...
import {
  RefreshCw, Mail, Zap, Inbox, Clock,
  AlertTriangle, CheckCircle2, Link2, Settings,
} from 'lucide-react';
...
function EmptyState({ onConnect }: { onConnect: () => void }) {
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
      ...
      <h3 className="text-lg font-bold text-gray-800 mb-2">No intake items yet</h3>
      <p className="text-gray-500 text-sm max-w-xs leading-relaxed mb-8">
        Connect your email and AI will automatically convert incoming legal
        correspondence into structured, actionable intake items.
      </p>
...
export default function EmailIntake() {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      const res = await intakeService.list(activeTab === 'all' ? undefined : activeTab);
      setItems(res.items);
    ...
    <div className="h-full flex flex-col -mx-8 -my-6 bg-white">
      ...
      <header className="h-16 border-b border-gray-200 flex items-center justify-between px-6 shrink-0 bg-white z-10">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-800 leading-tight">AI Intake Center</h1>
            <p className="text-xs text-gray-500">Emails → AI Processing → Structured actionable items</p>
          </div>
        </div>
            ...
            <div className="flex items-center gap-4 mr-4 text-sm font-medium">
              ...
              {summary.requires_action > 0 && (
                <span className="text-orange-500 flex items-center gap-1">
                  ...
                  {summary.requires_action} require action
                </span>
              )}
            </div>
          )}
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-[380px] shrink-0 border-r border-gray-200 bg-white flex flex-col shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
          ...
          <div className="flex-1 overflow-y-auto pt-3">
            {loading ? (
              ...
              <EmptyState onConnect={() => (window.location.href = '/settings')} />
            ) : items.length === 0 ? (
        ...
        <div className="flex-1 relative overflow-hidden">
          {selectedId ? (
            <IntakeDetailPanel
              itemId={selectedId}
              onConfirmed={() => { setSelectedId(null); load(true); }}
            ...
            <div className="flex-1 h-full flex flex-col items-center justify-center text-gray-400 bg-slate-50">
              ...
              <p className="text-sm font-medium text-gray-500">Select an intake item to review</p>
              <p className="text-xs text-gray-400 mt-1">AI analysis and actions will appear here</p>
            </div>
          )}
        </div>
